...
contract Compliance is Ownable {
    ...
    /// `createTemplate` is a simple function to create
    ///  a new compliance template
    ...
    /// @param _finalizes Timestamp of when the template will finalize
    ///  and become non-editable
    /// @param _expires Timestamp of when the template will expire
    /// @param _fee Amount of POLY to use the template (held in
    ///  escrow until issuance)
    ...
    function addJurisdictionToTemplate(
        bytes32 _template, 
        bytes32[] _allowedJurisdictions, 
        bool[] _allowed
    ) public
    {
