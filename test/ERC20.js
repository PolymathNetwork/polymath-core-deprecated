...
contract('ERC20', accounts => {
  ...
  describe('transfer, balanceOf', async () => {
    it('should update balanceOf() after transfer()', async () => {
      ...
      assert.equal(
        (await token.balanceOf(owner)).toNumber(),
        balanceOwner - transferredFunds,
      );
      assert.equal(
        (await token.balanceOf(spender)).toNumber(),
        transferredFunds,
      );
    });
  ...
  describe('approve, allowance, transferFrom', async () => {
    it('should not allow transfer() without approval', async () => {
      await expectRevert(
        token.transferFrom(owner, spender, 1, { from: spender }),
      );
    });
    ...
    it('should allow approve() multiple times only after resetting spender allowance to 0 in between', async () => {
      ...
      assert.equal(
        (await token.allowance(owner, spender)).toNumber(),
        allowedAmount + 1,
      );
    ...
    it('should not allow transfer() over approve() amount', async () => {
      ...
      await expectRevert(
        token.transferFrom(owner, to, allowedAmount, { from: spender }),
      );
    ...
    it('should update balanceOf() after transferFrom()', async () => {
      ...
      assert.equal(
        (await token.balanceOf(owner)).toNumber(),
        balanceOwner - allowedAmount / 2,
      );
      assert.equal((await token.balanceOf(spender)).toNumber(), balanceSpender);
      assert.equal(
        (await token.balanceOf(to)).toNumber(),
        balanceTo + allowedAmount / 2,
      );
    });
  ...
  describe('events', async () => {
    ...
    it('should log Transfer event after transferFrom()', async () => {
      ...
      let result = await token.transferFrom(owner, to, value, {
        from: spender,
      });
