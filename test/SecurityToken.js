...
describe("Compliance contracts functions", async()=> {
  ...
  it("proposeTemplate: should fail in proposing the template -- same template proposed again", async()=> {
 ...
 /////////////////////////
  ...
  it("proposeOfferingFactory: Should successfully propose the offering factory", async() =>{
    ...
    let txReturn = await compliance.proposeOfferingFactory(
      ...
      console.log('Proposal Index:',txReturn.logs[0].args._offeringFactoryProposalIndex.toNumber());
      txReturn.logs[0].args._offeringFactory.should.equal(offeringFactory_2.address);
  });

  it("proposeOfferingFactory: Should fail in proposing the offering factory -- deploying same factory again", async() =>{
    try {
    // Propose the Offering factory to a particular Security Token
    let txReturn = await compliance.proposeOfferingFactory(
      securityToken.address,
...
it("cancelOfferingFactoryProposal: Should fail in canceling the proposal -- msg.sender unauthorized",async() =>{
  ...
  async()=>{
    ...
    txReturn.logs[0].args._contributor.should.equal(investor1);
    try {
      let txReturn = await offeringContract.buy(15000, { from : investor1 , gas : 400000 });
    } catch(error) {
        ensureException(error);
    }
  });
